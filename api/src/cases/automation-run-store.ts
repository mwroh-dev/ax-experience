...
const AutomationRunRowSchema = z.object({
  ...
  prompt_version_id: z.string().nullable().optional().transform(v => v ?? undefined),
  input_hash: z.string(),
  output_summary: z.string().nullable().optional().transform(v => v ?? undefined),
  evidence_source_ids: z.string(),
  status: z.enum(['success', 'error', 'skipped']),
  latency_ms: z.number().nullable().optional().transform(v => v ?? undefined),
  error: z.string().nullable().optional().transform(v => v ?? undefined),
  created_at: z.string(),
});
