 ...
 *
 * 사용법 (프로젝트 루트에서):
 *   cd api && npx ts-node -r tsconfig-paths/register ../scripts/seed-rag-docs.ts
 *   OPENAI_API_KEY=sk-... cd api && npx ts-node -r tsconfig-paths/register ../scripts/seed-rag-docs.ts
 *
 * OPENAI_API_KEY 없으면 결정론적 mock 임베더로 실행됨.
 */
import fs from 'fs';
...
async function main(): Promise<void> {
  process.env.DB_PATH = process.env.DB_PATH ?? path.resolve(__dirname, '../.data/cs-ops.db');
  ...
  const embed = api_key
    ? make_openai_embedder(api_key)
    : (() => {
        console.warn('[seed] OPENAI_API_KEY not set — using mock embedder (no semantic meaning)');
        return make_mock_embedder(1536);
      })();
